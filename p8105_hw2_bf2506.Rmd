...
date: "2022-09-29"
---
```{r load package, message=FALSE, warning=FALSE, include=FALSE}
library(tidyverse)
library(magrittr)
library(lubridate)
```
...
```{r read and clean  snp_data, echo=FALSE, message=FALSE, warning=FALSE}

snp_data =
  read_csv("./data/snp.csv") %>%
  janitor::clean_names() %>% 
  mutate(date = parse_date_time(date, order = c("ymd", "mdy"))) %>% 
